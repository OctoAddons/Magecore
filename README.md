...
- The **Consumable** tab can maintain one **Mana Agate** in your bags. When enabled, an out-of-combat Rotation press casts Conjure Mana Agate if the gem is missing; MageCore never consumes it automatically.
...
- On the **Buffs** tab, check **Cast in group** beneath Intellect buffs you want applied to nearby party or raid members. This works through both the Buff button and Rotation's **Auto Buff OOC** mode.
...
Combat priority is mana-aware: unaffordable rotation spells are skipped. When current mana is below the cheapest learned spell configured in the six Rotation slots, MageCore falls back to **Shoot** with an equipped wand. It starts the wand through a native Shoot action slot when available and otherwise casts Shoot from the spellbook. Further Rotation presses do not toggle off an already-active wand attack.
